...
static void connect_wifi(const char *ssid, const char *password)
{
    ...
    assert(sta_netif);
    ESP_LOGI(TAG, "entra a task scan and connect");
...
static void echo_task(void *arg)
{
    ...
    ESP_ERROR_CHECK(uart_set_pin(ECHO_UART_PORT_NUM, PC_UART_TX_PIN, PC_UART_RX_PIN, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
    ...
    // obtiene los parametros pasados por la tarea
    bool *myArgs = (bool *)arg;
            ...
            if (str_mem[index - 1] == 13)
            {
                ...
                if (stringLen >= 1)
                {
                    ...
                    if (!strcmp(arrayString[0], "-i") || (!strcmp(arrayString[0], "-I") && stringLen > 2))
                    {
                        ...
                        connect_wifi(arrayString[1], arrayString[2]);
                    }
...
void app_main(void)
{
    ...
    xTaskCreatePinnedToCore(fast_scan, "wifi_connect_task", ECHO_TASK_STACK_SIZE, (void *)&trigger, 10, NULL, CORE_1);
    //wifi_scan();
    xTaskCreatePinnedToCore(echo_task, "uart_echo_task", WIFI_TASK_STACK_SIZE, (void *)&trigger, 10, NULL, CORE_2);
    //esp_help_log();
}
